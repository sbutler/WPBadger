...
add_filter( 'admin_post_thumbnail_html', 'wpbadger_badgedesigner_admin_post_thumbnail_html', 10, 2 );
add_action( 'wp_ajax_wpbadger_badgedesigner_publish', 'wpbadger_ajax_badgedesigner_publish' );
 ...
 */
function _wpbadger_badgedesigner_ajax_cleanup( $file )
{
    @unlink( $file );
}
 ...
 */
function _wpbadger_badgedesigner_media_handle_upload( $_f, $post_id, $post_data = array(), $overrides = array( 'test_form' => false ) )
{
    ...
    $name = $_f[ 'name' ];
    $file = _wpbadger_badgedesigner_wp_handle_upload( $_f, $overrides, $time );
...
/**
 * Handle errors inside of _wpbadger_badgedesigner_wp_handle_upload. Just returns an array with the
 * error message.
 */
function _wpbadger_badgedesigner_wp_handle_upload_error( &$file, $message )
{
 ...
 */
function _wpbadger_badgedesigner_wp_handle_upload( &$file, $overrides = false, $time = null )
{
...
function wpbadger_badgedesigner_openbadges_pre_upload( $post_id, $raw_badge, &$post_data, &$stash )
{
    ...
    $_f = array(
        'tmp_name'  => tempnam( sys_get_temp_dir(), 'wpbadger-badgedesigner-' ),
        'type'      => $image_hdr[ 0 ],
        'error'     => 0,
        'size'      => strlen( $image_data ),
    );
    $_f[ 'name' ] = pathinfo( $_f[ 'tmp_name' ], PATHINFO_BASENAME ) . '.png';
    register_shutdown_function( '_wpbadger_badgedesigner_ajax_cleanup', $_f[ 'tmp_name' ] );
...
function wpbadger_badgedesigner_openbadges_post_upload( $post_id, $attachment_id, &$stash )
{
    ...
    unset( $badge[ 'image' ] );
    add_post_meta( $attachment_id, 'wpbadger-badgedesigner-openbadges.me-data', $badge, true );
}
...
function wpbadger_ajax_badgedesigner_publish()
{
    ...
    {
        case 'openbadges.me':
            $_f = wpbadger_badgedesigner_openbadges_pre_upload(
                $post_id,
    ...
    $attachment_id = _wpbadger_badgedesigner_media_handle_upload(
        $_f,
        $post_id,
        $post_data,
        array(
    ...
    add_post_meta( $attachment_id, 'wpbadger-badgedesigner-source', $badge_source, true );
    switch ($badge_source)
    {
        case 'openbadges.me':
            wpbadger_badgedesigner_openbadges_post_upload( $post_id, $attachment_id, $stash );
            break;
    }
